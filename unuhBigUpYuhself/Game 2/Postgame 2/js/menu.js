(function() {
  ...
  Menu.prototype = {
    ...
    create: function () {
      ...
      this.moduleCount = 1;
      this.moduleText = this.add.text(this.game.width/2,this.game.height/2,"PREGAME 2" ,{font:'LondrinaSolid-Black'});
    ...
    update: function () {
      ...
      switch(this.moduleCount){        
        case 1:
          this.lock.alpha = 0;
          this.moduleText.text = "POSTGAME 2"
    ...
    onDown4: function () {
      ...
      if(this.intro.alpha == 1){
        ...
        try{
          if (messageHandler && parseInt(localStorage.getItem("moduleComplete_1B")) == 1) {
            //tokens no longer additive
            var tokenHolder = parseInt( localStorage.getItem("tokenCount"))
            localStorage.setItem("tokenCount",0);      
            localStorage.removeItem("moduleComplete_1B")                  
