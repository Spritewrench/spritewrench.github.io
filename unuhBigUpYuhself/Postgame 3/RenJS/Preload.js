var preload = {
  ...
  var preloadStory = {
    ...
    preload: function () {
      ...
      _.each(RenJS.setup.backgrounds,function(filename,background){
          ...
          if (str.length == 1){
              if(background.includes("avatarBedroom")){
                  game.load.image(background, "assets/backgrounds/avatarBedroom_"+localStorage.getItem("gender")+".png");
              }
              else{
      ...
      _.each(RenJS.setup.cgs,function(cgs,key){
          if (typeof cgs === 'string' || cgs instanceof String){
              // normal cgs
              if(key.includes("avatarCG")){
                  game.load.image(key, "assets/characters/char"+localStorage.getItem("avatar")+".png");
              }        
              else{
      ...
      _.each(RenJS.setup.characters,function(character,name){
          _.each(character.looks,function(filename,look){
              if(name.includes("avatar")){
                  game.load.image(name+"_"+look, "assets/characters/char"+localStorage.getItem("avatar")+".png");
              }
              else if(name.includes("friend")){
                  console.log(localStorage.getItem("avatar"))
                  var avatarChar = localStorage.getItem("avatar")
                  var text = avatarChar.split("_");
                  var charID = parseInt(text[0])+1
                  if(charID > 6){
                      charID= 1;
                  }
                  game.load.image(name+"_"+look, "assets/characters/char"+charID+"_"+text[1]+".png");
              }                
              else{
  ...
  function createStartingPage(){
    this.splash = game.add.sprite(game.world.centerX, game.world.centerY, 'splash-init');
    this.splash.anchor.set(0.5);
    if (globalConfig.splash.loadingBar) {
        var position = globalConfig.splash.loadingBar.position;
        this.loadingBar = game.add.sprite(position.x,position.y , "loading");
        if (this.loadingBar.animations.frameTotal > 1){
            // load second frame as full bar
            this.loadingBar.frame = 1;
        }
        game.add.tween(this.loadingBar).to({alpha:0}, 250, null,true,0,true,true).loop(true);
    }
  }
