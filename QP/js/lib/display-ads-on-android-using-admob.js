...
function getAds(){
  ...
  if( parseInt(localStorage.getItem("adReady")) == 0 ){
  ...
  document.addEventListener('admob.reward_video.reward',
    function (event) {
      ...
      if(getTix >= 1 && getTix <= 4){
        ...
        localStorage.setItem(deadMonster[0][getTix].commonRewards[2]+" Count",parseInt(localStorage.getItem(deadMonster[0][getTix].commonRewards[2]+" Count"))+5);
        ...
        localStorage.setItem(deadMonster[1][getTix-4].commonRewards[2]+" Count",parseInt(localStorage.getItem(deadMonster[1][getTix-4].commonRewards[2]+" Count"))+5);        
        var amount = purchaseLimit-1
        if(amount <= 0){
          amount = 0
        }
        ...
        localStorage.setItem(deadMonster[2][getTix-8].commonRewards[2]+" Count",parseInt(localStorage.getItem(deadMonster[2][getTix-8].commonRewards[2]+" Count"))+5);              
        var amount = purchaseLimit-1
        if(amount <= 0){
          amount = 0
        }
