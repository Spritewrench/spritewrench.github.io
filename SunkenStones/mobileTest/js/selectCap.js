(function() {
  ...
  selectCap.prototype = {
    ...
    create: function () {
      ...
      // pity captain cesar unlocks
      const date = new Date();
      const pityDeadline = new Date('2025-08-31T12:00:00.000000Z')   

      if(pityDeadline > date){
        try{
          if(client.achievement.isActivated('CLEAR_ZONE_2') && parseInt(localStorage.getItem("cap_unlocked3")) == 1 && parseInt(localStorage.getItem("cap_unlocked4")) != 1){
      ...
      this.bubbleTxt = this.add.text(265,y+250, 'WELCOME!\nFANCY ANOTHER\nGAME?', {font: '32px Kaph-Regular',fill: '#000', align: 'center'});
      ...
      */
      ...
      this.unlockedCrew.deployText = this.add.text( this.unlockedCrew.x-(size/2)+33,this.unlockedCrew.y+(size/2)-25, 'X', {font: '42px LondrinaSolid-Black',fill: '#fff', align: 'center'});
      ...
      this.unlockedCrew.powerText = this.add.text(this.unlockedCrew.x+(size/2)-31,this.unlockedCrew.y+(size/2)-25, 'X', {font: '42px LondrinaSolid-Black',fill: '#fff', align: 'center'});
      ...
      */
      ...
      localStorage.setItem("prevScene","selectCap")

      if(prevScene.includes("menu")){
        this.changeWaveDown();
        this.boardView = this.add.sprite(0,0, 'desk1');
        this.boardView.width = this.game.width 
        this.boardView.height = this.game.height
        this.boardView.alpha = 0;          
      }

      if(prevScene.includes("game")){
          this.boardView = this.add.sprite(0,0, 'desk1');
          this.boardView.width = this.game.width 
          this.boardView.height = this.game.height      

          var tweenA = this.add.tween(this.boardView).to( { alpha: 0 }, 400, Phaser.Easing.Default);   
          this.bg2.x = 0
          var tweenB = this.add.tween(this.bg2).to({ x:300 },400, Phaser.Easing.Default); 
      ...
      var unlockArray = localStorage.getItem("unlockedCrewList")
      console.log(unlockArray)
      if(unlockArray.length>0){
        this.unlockList = unlockArray.split(',')
        var CaptainUnlocked = parseInt(localStorage.getItem("cap_unlocked2New"))+parseInt(localStorage.getItem("cap_unlocked3New"))+parseInt(localStorage.getItem("cap_unlocked4New"))+parseInt(localStorage.getItem("cap_unlocked5New"))+parseInt(localStorage.getItem("cap_unlocked6New"))
        
        if(this.unlockList[0].length > 0 && (CaptainUnlocked %2 == 0 || isNaN(CaptainUnlocked) )){
          console.log("Checking list")
          this.unlockList.pop()
          this.unlockCrew(this.unlockList.pop())
        }        
      }
      else{
        console.log("no list")
    ...
    update: function () {
      ...
      */
      ...
      this.capVOSnd[1].volume = (parseInt(localStorage.getItem("sfxVol"))* 0.1)
      this.capVOSnd[2].volume = (parseInt(localStorage.getItem("sfxVol"))* 0.1)
      this.capVOSnd[3].volume = (parseInt(localStorage.getItem("sfxVol"))* 0.6)
      ...
      if(this.selectCapSelect.alpha == 1 && this.unlockList.length <= 0){
      ...
      if(this.transWave.y <= this.game.height&& this.transWaveKey == 0){
        ...
        if(this.transWave.y >= this.game.height&& this.transWaveKey == 0){
          ...
          if (parseInt(localStorage.getItem("intro")) == 13) {
            ...
            Swal.fire({
              title: 'Well Done!',          
              html: "You've completed your first voyage!",
              ...
              if (result.isConfirmed) { 
                try{
                  ...
                  Swal.fire({
                    title: 'Well Done!',          
                    html: 'You have unlocked a new Captain!<br/>New crewmates can be also discovered on your voyage',
                    imageUrl: 'assets/mascot.webp',
                    imageWidth: 200,
                    imageHeight: 200,
                    allowOutsideClick: false,
          ...
          if (parseInt(localStorage.getItem("cap_unlocked2New")) == 1) {
            ...
            Swal.fire({
              title: 'Well Done!',          
              html: 'You have unlocked a new Captain!<br/>New crewmates can be also discovered on your next voyage',
              imageUrl: 'assets/mascot.webp',
              imageWidth: 200,
              imageHeight: 200,
              allowOutsideClick: false,
              ...
              if (result.isConfirmed) {
                var ran = Math.floor(Math.random() * 3)+1;
                this.jingleSnd[ran].play();    
                localStorage.setItem("unlockedCrewList","13,27,28,9,10,21,8,16,20,")
                var unlockArray = localStorage.getItem("unlockedCrewList")
                this.unlockList = unlockArray.split(',')              
                this.unlockList.pop()  
                this.unlockCrew(this.unlockList.pop())        
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                var ran = Math.floor(Math.random() * 3)+1;
                this.jingleSnd[ran].play();            
                localStorage.setItem("unlockedCrewList",localStorage.getItem("unlockedCrewList")+"15,25,26,11,4,19,6,7,17,")
                var unlockArray = localStorage.getItem("unlockedCrewList")
                this.unlockList = unlockArray.split(',')        
                this.unlockList.pop()        
                this.unlockCrew(this.unlockList.pop())
          ...
          else if (parseInt(localStorage.getItem("cap_unlocked4New")) == 1) {
            localStorage.setItem("cap_unlocked4New", 2)
            Swal.fire({
              title: 'Well Done!',          
              html: 'You have unlocked a new Captain!',
              imageUrl: 'assets/mascot.webp',
              imageWidth: 200,
              imageHeight: 200,
              allowOutsideClick: false,
          ...
          else if (parseInt(localStorage.getItem("cap_unlocked5New")) == 1) {
            localStorage.setItem("cap_unlocked5New", 2)
            Swal.fire({
              title: 'Well Done!',          
              html: 'You have unlocked a new Captain!',
              imageUrl: 'assets/mascot.webp',
          ...
          else if (parseInt(localStorage.getItem("cap_unlocked6New")) == 1) {
            localStorage.setItem("cap_unlocked6New", 2)
            Swal.fire({
              title: 'Well Done!',          
              html: 'You have unlocked a new Captain!',
              imageUrl: 'assets/mascot.webp',
...
heightAuto: false                        
    ...
    unlockCrew: function (crewId) {
      
    this.bagWiggle = 0;
    var unlockCrew = crewId//Math.floor(Math.random() * 78)+1;
      ...
      this.unlockedCrew.loadTexture("bigCrew-"+unlockCrew)
      ...
      this.unlockedCrew.name.text = crew[unlockCrew].name+"\nTYPE: "+crewType
        ...
        this.unlockedCrew.deployText.x = this.unlockedCrew.x-(this.unlockedCrew.width/2)+80
        this.unlockedCrew.powerText.x = this.unlockedCrew.x+(this.unlockedCrew.width/2)-80
        this.unlockedCrew.ability.x = this.unlockedCrew.x
        this.unlockedCrew.name.x = this.unlockedCrew.x           
        
        this.unlockedCrew.deployText.y = this.unlockedCrew.y-(this.unlockedCrew.height/2)+145
        this.unlockedCrew.powerText.y = this.unlockedCrew.y-(this.unlockedCrew.height/2)+135
        this.unlockedCrew.ability.y = this.unlockedCrew.y+195
        this.unlockedCrew.name.y = this.unlockedCrew.y +80          
    ...
    hideUnlock: function () {
      ...
      var unlockedSoFar = getCrewUnlockedLength(); 
      if(unlockedSoFar >= 22){
        try{
          if (!client.achievement.isActivated('UNLOCK_CREW_22')) {
            client.achievement.activate('UNLOCK_CREW_22')
          } 
        }
        catch(e){ }  
      }
      if(unlockedSoFar >= 34){
        try{
          if (!client.achievement.isActivated('UNLOCK_CREW_34')) {
            client.achievement.activate('UNLOCK_CREW_34')
          } 
        }
        catch(e){ }  
      }
      if(unlockedSoFar >= 48){
        try{
          if (!client.achievement.isActivated('UNLOCK_CREW_48')) {
            client.achievement.activate('UNLOCK_CREW_48')
          } 
        }
        catch(e){ }  
      }
    ...
    lookAtBoard: function(start,end){
      var animSpeed = 25
      var tweenA = this.add.tween(this.boardView).to({ alpha:1 }, 400, Phaser.Easing.Default); 
      var tweenB = this.add.tween(this.bg2).to({ x:0 },400, Phaser.Easing.Default); 
      this.boardView.loadTexture("desk"+(start))  
      
      if(this.boardView.alpha == 0){
        tweenA.onComplete.addOnce(function(){
          this.timer = this.game.time.create(true);  
          this.timer.add(animSpeed, function(){       
          if(start < end){
            start += 1
            this.lookAtBoard(start,end)
          }
          else if(start > end){
            start -= 1
            this.lookAtBoard(start,end)
    ...
    onDown: function () {
      console.log(this.unlockList)
      console.log(this.unlockList.length)
      console.log(this.selectCapSelect.alpha)
      if(this.selectCapSelect.alpha != 0 && this.unlockList.length <= 0){
        var ran = Math.floor(Math.random() * 3)+1;
        this.jingleSnd[ran].play();   
        var ran = Math.floor(Math.random() * 3)+1;
        this.wavSnd[ran].play() 
