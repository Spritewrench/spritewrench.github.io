  ...
  Menu.prototype = {
    ...
    create: function () {
     ...
     // this.scoreCount = this.add.text(this.scoreCountUI.x,this.scoreCountUI.y, localStorage.getItem("highScore"), {font: '32px Kaph-Regular',fill: '#fff', align: 'left'});
      ...
      this.titleTxt4 = this.add.text(x,this.game.height/2+75, 'PLAY', {font: '40px Kaph-Regular',fill: '#fff', align: 'center'});
      ...
      this.titleTxt8 = this.add.text(x,this.game.height/2+150, 'COLLECTON', {font: '40px Kaph-Regular',fill: '#fff', align: 'center'});
      ...
      this.titleTxt5 = this.add.text(x,this.game.height/2+225, 'OPTIONS', {font: '40px Kaph-Regular',fill: '#fff', align: 'center'});
      ...
      this.titleTxt6 = this.add.text(x,this.game.height/2+300, 'CREDITS', {font: '40px Kaph-Regular',fill: '#fff', align: 'center'});
      ...
      this.titleTxt7 = this.add.text(x,this.game.height/2+375, 'EXIT', {font: '40px Kaph-Regular',fill: '#fff', align: 'center'});
      ...
      this.titleTxt10 = this.add.text(x,this.game.height-25, 'VERSION: '+version+"", {font: '20px Kaph-Regular',fill: '#fff', align: 'center'});
      ...
      /*
      this.inDev = this.add.sprite(0, -15, 'inDev-Popup');
      this.inDev.width = this.game.width
      this.inDev.height = this.game.height
      */
      ...
      //retooled insta link
      this.steamComm = this.add.sprite(this.game.width-275, this.game.height-75, 'social_steam');
      this.steamComm.width = 100;
      this.steamComm.height = 100;
      this.steamComm.anchor.setTo(0.5, 0.5);
      this.steamComm.origWidth = this.steamComm.width;
      this.steamComm.origHeight = this.steamComm.height
      this.steamComm.inputEnabled = true;
      this.steamComm.events.onInputOver.add(this.spinUp, this);
      this.steamComm.events.onInputOut.add(this.spinDown, this);    
      this.steamComm.events.onInputDown.add(this.opensteamComm, this);  
      //this.steamComm.events.onInputDown.add(this.openInsta, this);        

      this.social_threads = this.add.sprite(this.game.width-400, this.game.height-75, 'social_discord');
      ...
      this.social_threads.events.onInputDown.add(this.openDiscord, this);          
      ...
      this.bgSound = this.add.audio('mainTheme');
      ...
      */
      ...
      if (localStorage.getItem("intro") === null || parseInt(localStorage.getItem("intro")) < 14) {
        localStorage.setItem("intro",1);
        localStorage.setItem("captain",1);
        localStorage.setItem("afflictTut",0);
        localStorage.setItem("curseTut",0);
        localStorage.setItem("bossTut",0);
      }  
       ...
       if (localStorage.getItem("tutorialStart") === null) {
         localStorage.setItem("tutorialStart",1);
       }   
       ...
       Swal.fire({
        title: 'Ahoy, Captain! ',
        html: "Thank you for sailing with us! <br/><br/>Sunken Stones is currently in active development, and as part of this journey, you may encounter placeholders, rough edges, and the occasional sea monster... er, bug.",
        backdrop: false,
        ...
        */  
       //this.changeWaveDown();
      if(localStorage.getItem("prevScene") !== null ){
        var prevScene = localStorage.getItem("prevScene")
        localStorage.setItem("prevScene","menu")     
        if(prevScene.includes("selectCap") && prevScene !== null ){
          this.changeWaveDown();          
        }  
        
        if(prevScene.includes("tutorial") && prevScene !== null ){
          this.changeWaveDown();          
        }        

        if(prevScene.includes("collection") && prevScene !== null ){
          ...
          this.titleTxt8.text = "COLLECTION ("+unlockedCrewNum+"/"+crewNum+")"        
        }  
      }
      else{
        localStorage.setItem("prevScene","menu")
    ...
    update: function () {
      ...
      if(parseInt(localStorage.getItem("emailGiven")) == 1 || !window.navigator.onLine ){
        this.steamComm.y  = this.game.height+200
      }
      else{
        this.steamComm.y  = this.game.height-75
      }

      this.bgSound.volume = parseInt(localStorage.getItem("bgVol"))* 0.2  
      ...
      if(this.transWaveKey == 1){
        ...
        if(this.transWave.y <= (-this.game.height+50)){
          ...
          if(this.transTar.includes("selectCap") && parseInt(localStorage.getItem("intro")) <= 12){
    ...
    goToCollection: function () {
      ...
      if(this.titleTxt4.alpha >= 1 && parseInt(localStorage.getItem("intro")) >= 14){
        var ran = Math.floor(Math.random() * 3)+1;
        ...
        if(this.count >= 3){
          ...
          this.transWaveKey = 1;
          this.changeWave();
          this.transTar = 'collection'
         //this.game.state.start('selectCap');
        }    
      }  
      else if(this.titleTxt4.alpha >= 1){
        //this.transWaveKey = 1;
        //this.transTar = 'win'
        
        Swal.fire({
          title: 'Not Available Yet!',
          text: "This feature is locked.Complete Tutorial to unlock",
          backdrop: false,
          allowOutsideClick: false,
          allowEscapeKey: false ,
          heightAuto: false                        
    ...
    showItems: function () {
      ...
      if(parseInt(localStorage.getItem("intro")) < 14){
         this.titleTxt8.text = "COLLECTION (LOCKED)"
      }        
...
//;const electron = require("electron");const ipc = electron.ipcRenderer;ipc.send("toggle-maximize-window");
    ...
    showCredits: function () {
      if(this.titleTxt7.alpha >= 1){
        ...
        Swal.fire({
          title: 'CREDITS',
          html: "<h3>ART & PROGRAMMING</h3>Glen Henry<h3><h3>STORE KEY & CAPSULE ART</h3>DesignSunset<h3>AUDIO</h3>Music Composed by Chase Bethea<br/>VO Audio produced by Dillon Becker <h3>ANIMATION</h3>Nathanael Hay<h3>VOICE WORK</h3>Ayanna Henry<br/>Charis Cayetano-Henry<br/>Marishka Simms<br/>Meghan Christian<br/>Alex Brodie<br/>Kevin Jackson<br/>John Hazel<br/>Voice Actor's League Ja(TM)<br/><h3>MARKETING & PR</h3>Benne Media Solutions<br/><h3>QUALITY ASSURANCE</h3>Alrick Brown, Graham Reid<br/>Matthew Hunter, Paul Griffiths<br/>Shaun Douglas, Evon Binns<br/><h3>SPECIAL THANKS</h3>Damien Crawford<br/> <h3>PATREON SUPPORTERS</h3>Anchored Digital<br/>",
          allowOutsideClick: false,
          allowEscapeKey: false,
          heightAuto: false          
        })          
